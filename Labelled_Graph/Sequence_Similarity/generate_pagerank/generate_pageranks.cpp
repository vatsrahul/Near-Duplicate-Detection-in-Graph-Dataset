...
int main(int argc, char const *argv[])
{
	...
	for(int g_ind = 0; g_ind < graph_dataset.size(); g_ind++)
	{
		...
		{
			pagerank_file << "v " << graph_dataset[g_ind].vertices[vtx_ind].vid << " "<< (char)('A'+graph_dataset[g_ind].vertices[vtx_ind].label) << " " << graph_dataset[g_ind].vertices[vtx_ind].quality << endl;
		}
	}
	pagerank_file.close();
