...
void Graph::displayGraph()   // no changes made here
{
	cout <<"g "<< gid << ":" << endl;
...
void Graph::walkAlgorithm()	// only chnge is walk conatins LABEL and not VID
{
	int ind = 0;
	int walkInd = 0;
	...
	while(ind < vertexCount)
	{
		...
		walk[walkInd++] = vid_to_label[curr_vtx]; // adding current vertex to walk
