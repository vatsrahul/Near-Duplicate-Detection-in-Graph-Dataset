...
int main(int argc, char const *argv[])
{
	...
	unsigned long loose_filter_count=0,strong_filter_count=0, simPairCount=0, candidate_graph_count=0;
...
void loadPageRanks(ifstream &pagerank_file, vector<Graph> &graph_dataset, int &dataset_size)
{
	...
	for(int graph_ind=0; graph_ind < dataset_size; graph_ind++)
	{
		...
		{
			// each line for each vertex should be in the format like: "v vid(unsigned) label vertex quality(double)"
			pagerank_file >> tag >> vtx_id >> vc >> vtx_quality;
