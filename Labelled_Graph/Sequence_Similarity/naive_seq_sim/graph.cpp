...
void Graph::displayGraph(){  // not making any change here
...
void Graph::walkAlgorithm()   // ONLY CHANGE HERE IS THAT INSTEAD OF VID VERTEX LABEL IS STORED IN "WALK"
{
	int ind = 0;
	int walkInd = 0;
	...
	while(ind < vertexCount)
	{
		...
		walk[walkInd++] = vid_to_label[curr_vtx]; // adding current vertex label to walk
...
void Graph::computeShingles()  // NO CHANGE AS WALK ALREADY CONTAINS LABELS ONLY
{
