...
int main(int argc, char const *argv[])
{
	...
	for(int g_ind = 0;g_ind < graph_dataset.size(); g_ind++)
	{
		...
		{
			sorted_dataset_file << "v " << graph_dataset[g_ind].vertices[vtx_ind].vid <<" "<< (char)('A'+graph_dataset[g_ind].vertices[vtx_ind].label) << endl;
		}

		unordered_map<unsigned, char> vid_to_label = graph_dataset[g_ind].vid_to_label; 

		for(int edge_ind = 0;edge_ind < graph_dataset[g_ind].edgeCount; edge_ind++)
		{
			sorted_dataset_file << "e " << graph_dataset[g_ind].edges[edge_ind].first << " " <<vid_to_label[graph_dataset[g_ind].edges[edge_ind].first] <<" "
										<< graph_dataset[g_ind].edges[edge_ind].second << " " <<vid_to_label[graph_dataset[g_ind].edges[edge_ind].second] << endl;	
		}
	}
