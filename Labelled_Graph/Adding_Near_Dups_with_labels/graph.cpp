...
void Graph:: pushEdge(unsigned src_vtx,unsigned dest_vtx){		// vid are pushed
	if(src_vtx > dest_vtx)
...
void Graph:: readGraph(istream &inp){
	...
	for(int e_ind = 0; e_ind < edgeCount; e_ind++)
	{
		...
		pushEdge(src_vtx, dest_vtx);			//NOTICE LABEL IS PASSED
	}
}
...
void Graph::displayGraph()
{
	...
	for(int edg_ind = 0; edg_ind<edgeCount; edg_ind++)
	{
		//cout << "e" << edg_ind << ": " << edgeo[edg_ind].first << " " << edgeo[edg_ind].second << endl;
	}
}
*/